...
const getEnhancedContent = (htmlContent: string) => {
  ...
  return htmlContent
    .replace(/text-xs sm:text-\[14.5px\] text-left/g, 'text-[16.5px] sm:text-[18.5px] leading-relaxed sm:leading-loose font-medium')
    .replace(/text-xs sm:text-\[13.5px\] text-slate-655/g, 'text-[16.5px] sm:text-[18.5px] text-slate-700 leading-relaxed sm:leading-loose')
    .replace(/text-xs sm:text-\[13.5px\] text-slate-650/g, 'text-[16.5px] sm:text-[18.5px] text-slate-700 leading-relaxed sm:leading-loose')
    .replace(/text-xs sm:text-\[13.5px\]/g, 'text-[16.5px] sm:text-[18.5px] leading-relaxed')
...
export default function SuccessColumnPage({ onBack, onSelectPlan, initialTab = 'matcher', initialArticleId = null }: SuccessColumnPageProps) {
  ...
  // Pagination states (10 items per page standard)
  const [currentPage, setCurrentPage] = useState(1);

  // Reset page when any search filters change to prevent out of bounds paging
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedJob, selectedDebt, searchQuery]);
                      ...
                      <button
                        onClick={() => {
                          if (currentPage > 1) {
                            ...
                            document.getElementById('success-stories-list-anchor')?.scrollIntoView({ behavior: 'smooth' });
                          }
                        }}
                        disabled={currentPage === 1}
                        className="w-8 h-8 rounded-lg flex items-center justify-center border border-slate-200 bg-white text-slate-500 hover:border-amber-500 hover:text-amber-600 disabled:opacity-40 disabled:hover:border-slate-200 disabled:hover:text-slate-500 transition-all cursor-pointer"
                      >
                        <ChevronLeft className="w-4 h-4" />
                      </button>
                        ...
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg font-black text-xs transition-all cursor-pointer ${
                            currentPage === page
                              ...
                              : 'bg-white border border-slate-200 text-slate-650 hover:border-amber-500 hover:text-amber-600'
                          }`}
                        >
                          {page}
                        </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 sm:p-8 overflow-y-auto space-y-6 leading-relaxed">
                ...
                {/* HTML prose content */}
                <div 
                  className="text-slate-750 font-medium text-[16px] sm:text-[18.5px] leading-relaxed sm:leading-loose space-y-5 prose max-w-none prose-slate whitespace-normal break-all text-left"
